...
impl Download {
    ...
    pub fn show(ui: &mut Ui, download: &mut Download) {
        ...
        ui.horizontal(|ui| {
            // Check status
            ui.checkbox(
                &mut download.check_status,
                "Check host status (will take a longer amount of time)",
            );
            // Generate links
            if ui.button("Generate download links").clicked() {
                ...
                let check_status = download.check_status;
                        ...
                        ui.add(
                            TextEdit::multiline({
                                download.display_download_links =
                                    filter_links(&download.direct_download_links, &download.filter);
