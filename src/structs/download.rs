...
impl Download {
                ...
                ScrollArea::vertical().min_scrolled_height(ui.available_height()).id_source("Direct Link Output Box").show(ui, |ui| {
                    ...
                    ui.vertical(|ui| {
                        ...
                        for link in self.display_links.iter() {
                            ...
                            //if self.selection_indices.0 != 0 && self.selection_indices.0 != self.selection_indices.1 {
                            //    slice[self.selection_indices.0-1..self.selection_indices.1].iter().for_each(|link| { selected_links.insert(link); });
                            //    if ui.ctx().input(|ui| !ui.modifiers.shift) {
                            //        self.selection_indices.0 = 0;
                            //    };
                            //};
    ...
    //    fn display_links_ui(&mut self, ui: &mut Ui) {
    //        let direct_links: Vec<DirectLink> = self.mirror_links.iter().flat_map(|link| link.direct_links.clone().unwrap()).collect();
    //        self.display_links = filter_links(&direct_links, &self.filter_menu);
    //        println!("{:?}", self.display_links);
    //        ui.horizontal(|ui| {
    ...
    //
    //                                let slice = &self.display_links;
    //                                let index_1 = self.selection_indices.0;
    //                                let index_2 = self.selection_indices.1;
    //                                if self.selection_indices.0 > self.selection_indices.1 {
    //                                    self.selection_indices.0 = index_2;
    //                                    self.selection_indices.1 = index_1;
    //                                };
    //
    //                                if self.selection_indices.0 != 0 && self.selection_indices.0 != self.selection_indices.1 {
    //                                    slice[self.selection_indices.0-1..self.selection_indices.1].iter().for_each(|link| { selected_links.insert(link); });
